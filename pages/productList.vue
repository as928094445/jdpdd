	...
	<view class="container">
		...
		<view class="tui-header-screen" :style="{top:height+'px'}">
			<view class="tui-screen-top">
				...
				<view class="tui-top-item" :class="[withCoupon?'tui-active tui-bold':'']" @tap="screen" data-index="1">优惠券</view>
			...
			<view class="tui-screen-bottom" v-show="false">
		...
		<view class="tui-product-list" :style="{marginTop:px(dropScreenH+18)}">
							...
							<view class="tui-pro-content">
								...
								<view>
                                    ...
                                    <view class="promotion" v-if="item.promotion_money">
                                        <tui-tag size="small" type="warning" shape="circle" tui-tag-class="tui-tag-coupon">推广预计可赚{{item.promotion_money}}元</tui-tag>  
                                    </view>
								</view>
							</view>
						</view>
			...
			<tui-tips :fixed="false" style="margin:0 auto;" v-if="productList.length==0 && !loadding" imgUrl="/static/images/toast/img_nodata.png">暂无数据</tui-tips>
		</view>		
		<!--list-->
			...
			<view class="tui-drawer-box" :style="{paddingTop:height+'px'}">
				<scroll-view class="tui-drawer-scroll" scroll-y :style="{height:drawerH+'px'}">
                    <view class="tui-drawer-title">
                    	<text class="tui-title-bold">筛选</text>
                    </view>
                    <view class="tui-drawer-content tui-flex-attr">
                    	<view class="tui-attr-item" :class="[isBrand?'tui-btmItem-active':'']" @tap="setBrand">
                    		<view class="tui-attr-ellipsis">品牌商品</view>
                    	</view>
                    </view>
                    
					<view class="tui-drawer-title">
						<text class="tui-title-bold">全部类别</text>
					</view>
					<view class="tui-drawer-content tui-flex-attr">
						<view class="tui-attr-item" :class="[item.selected?'tui-btmItem-active':'']" v-for="(item,index) in categoryArr" :key="index" @tap.stop="setOpt" :data-index="index">
				...
				<view class="tui-attr-btnbox">
					<view class="tui-attr-safearea">
						<view class="tui-drawer-btn tui-drawerbtn-black" hover-class="tui-white-hover" :hover-stay-time="150" @tap="resetDrawer">重置</view>
...
<script>
	...
	export default {
        computed:{
            ...mapState(['isLogin','userInfo','pid','allianceName'])
        },
		components: {
			tuiIcon,
			tuiDrawer,
		...
		methods: {
			...
			dropdownItem: function(e) {
				...
				this.selectedName = arr[index]['selectedName'];
				this.selectH = 0
				this.sortType = arr[index]['num'];
				this.pageIndex = 1;
				this.getSearchList();				
			},
			screen: function(e) {
				...
				if (index == 0) {
					...
					this.withCoupon = (this.withCoupon)?false:true;
                    this.pageIndex = 1;
                    this.getSearchList();
				} else if (index == 2) {
            ...
            setOpt:function(e){
                let index = e.currentTarget.dataset.index;
                let selected = this.categoryArr[index].selected;
                this.opt_id = this.categoryArr[index].opt_id;  
                console.log(this.opt_id);
                this.resetDrawer('opt');
                this.$set(this.categoryArr[index], "selected", !selected)
            },
            resetDrawer: function(type) {
                if(type!='opt'){
                    this.isBrand = false
                    this.opt_id = '';
                }
                for (let i = 0; i < this.categoryArr.length; i++) {
                    this.$set(this.categoryArr[i], "selected", false)
                }
            },
			...
			getSearchList:function(){
				this.loadding = true;
				let _this = this;
                console.log(_this.opt_id);
				util.getSearch({
                    'keyword':_this.searchKey,
                    'sortType':_this.sortType,
                    'withCoupon':_this.withCoupon,
                    'isBrand':_this.isBrand,
                    'opt_id':_this.opt_id,
                    'pages':_this.pageIndex,
                },(reData) => {
            ...
            getInfo:function(){
                let _this = this;
                _this.$http.get('/opt').then(res => {
            		if(res.data.ret==200){						
            			_this.categoryArr = res.data.data;
            		}					
            	}).catch(err => {
            		this.getInfo();
            	})
