	...
	<view class="navigation-bar" :style="{height:height}">
		<view class="navigation-bar-fixed" :style="{height:height,background:(!config.transparent&&!config.linear)?config.bgcolor:'transparent'}">
			...
			<view v-if="[1,2].indexOf(config.type)!= -1" :class="config.type==1?'navigation-bar-capsule':'navigation-bar-textbox'" :style="{top:marginTop}">
				<view class="button-action" hover-class="button-action-active" @click="allianceFun('jd')">
					<image :src="jdImg"></image>
				</view>
				<view class="button-v-line"></view>
				<view v-if="!config.share" class="button-action" hover-class="button-action-active" @click="allianceFun('pdd')">
					<image :src="pddImg"></image>
				</view>
				<view v-if="config.share" class="button-action" hover-class="button-action-active">
					<button class="button_clear" open-type="share" hover-class="none">
						<image src="/static/icon/share_.png"></image>
					</button>
				</view>
			</view>
			<!-- custom button -->
...
<script>
	...
	export default {
		data() {
			return {
				jdImg:'/static/icon/jd_at.png',
                pddImg:'/static/icon/pdd.png'
			};
		},
		computed:{
            ...mapState(['isLogin','userInfo','allianceType','allianceName']),
			height(){
				...
				return height+ 38 + "px" // 胶囊高度 32px 下边框6px height 状态栏高度
			},
			marginTop(){
				const {platform,statusBarHeight} = uni.getSystemInfoSync()
		...
		methods:{
            ...
            allianceFun(op){
                if(op=='pdd'){
                    this.jdImg = '/static/icon/jd.png';
                    this.pddImg = '/static/icon/pdd_at.png'
                }else{
                    this.jdImg = '/static/icon/jd_at.png';
                    this.pddImg = '/static/icon/pdd.png'
                }
            	this.$emit("allianceFun",op)
            }
		}
	}
</script>
